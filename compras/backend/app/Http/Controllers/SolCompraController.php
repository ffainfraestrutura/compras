...
class SolCompraController extends Controller
{
    public function store(Request $request)
    {
        ...
        ", [$request->cod_material]);
            ...
            // ✅ CORREÇÃO 1: Busca o cargo do solicitante (já corrigido com COLLATE)
            $cargoSolicitante = DB::selectOne("
            SELECT u.matricula, u.nome, u.email,
            CASE 
            ...
            END AS cargo_atual,
            g.idtbgerente, d.id, cl.id 
            FROM bdcorp.tbusuario u 
            LEFT JOIN bdcorp.tbgerente g ON g.matricula COLLATE utf8mb4_unicode_ci = u.matricula COLLATE utf8mb4_unicode_ci
            LEFT JOIN bdcorp.tbdiretor d ON d.matricula COLLATE utf8mb4_unicode_ci = u.matricula COLLATE utf8mb4_unicode_ci
            LEFT JOIN bdcorp.tbc_level cl ON cl.matricula COLLATE utf8mb4_unicode_ci = u.matricula COLLATE utf8mb4_unicode_ci
            WHERE u.matricula = ?
        ", [$solicitacao->solicitante]);
            ...
            if ($cargoSolicitante && $cargoSolicitante->tem_cargo_superior == 1) {
                ...
                // ✅ CORREÇÃO 2: Adiciona COLLATE na consulta de gestores de compras
                $usuariosCompras = DB::select("
                SELECT u.email, u.matricula, u.nome, gm.ids_gestao_material
                FROM bdcorp.tbusuario u
                JOIN bdcompra.tbgestores_material gm ON gm.matricula COLLATE utf8mb4_unicode_ci = u.matricula COLLATE utf8mb4_unicode_ci
                WHERE u.compras = 2
            ...
            ", [json_encode($centroCustoAlvo)]);
                ...
                // ✅ CORREÇÃO 3: Adiciona COLLATE na consulta do superior
                // Consulta do superior corrigida
                $superior = DB::selectOne("
                    SELECT 
                        ...
                        g.idtbgerente
                    FROM bdcorp.tbusuario u
                    LEFT JOIN bdcorp.tbcoord c ON CONVERT(c.matricula USING utf8mb4) COLLATE utf8mb4_unicode_ci = u.matricula COLLATE utf8mb4_unicode_ci
                    LEFT JOIN bdcorp.tbgerente g ON g.idtbgerente = c.idtbgerente
                    LEFT JOIN bdcorp.tbusuario ug ON ug.matricula COLLATE utf8mb4_unicode_ci = g.matricula COLLATE utf8mb4_unicode_ci
                    WHERE u.matricula = ?
                ", [$solicitacao->solicitante]);
                ...
                if ($superior && !empty($superior->email_superior)) {
                    ...
                    // ✅ CORREÇÃO 4: Adiciona COLLATE na consulta do gestor padrão
            ...
            $solicitacao->update([
                ...
                'matricula_gerente' => $solicitacao->solicitante,
                'justificativa_gerente' => 'Aprovado automaticamente pelo sistema'
            ]);
