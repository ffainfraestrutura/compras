...
export default function Index() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const endpoint =
                    groupBy === "cod_compra"
                        ? `${import.meta.env.VITE_API_URL}/aprovarcompra_gerente_material` // endpoint original
                        : `${import.meta.env.VITE_API_URL}/aprovarcompra_gerente_material`; // endpoint para materiais
    ...
    const handleSubmitApproval = async () => {
        ...
        try {
            ...
            await axios.put(`${import.meta.env.VITE_API_URL}/aprovarcompra_gerente_material`, payload, {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
            });
    ...
    if (nivelAcesso > 3 && nivelAcesso != 7) {
        tituloTabela = "Aprovar Solicitações";
    }

    // Supondo que você tenha o nível do usuário
    hiddenIndexes = hiddenIndexes.filter((index) => {
        if (userLevel == 3) {
            return index != 3 && index != 4; // remove apenas o elemento 3
        } else if ([5, 6].includes(userLevel)) {
            return index != 3 && index != 4 && index != 5; // remove elementos
        }
        return true; // mantém os outros casos
    });
