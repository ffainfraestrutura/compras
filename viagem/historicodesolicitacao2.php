                ...
                <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="mb-3">
                    <div class="row">
                        ...
                        <div class="form-group col-md-2">
                            ...
                            <button class="form-control form-control-md btn-sm btn-success"
                    ...
                    <table id='tabela1' class='table' style="font-size: 13px;">
                        <thead>
                            <tr>
                                ...
                                <th>Detalhes</th>
                                <th>Passagem Ida:</th>
                                <th>Passagem Volta:</th>
                                <th>Status</th>
                                <th class="d-none">Empresa Ida</th>
                                ...
                                <th class="d-none">Empresa Volta</th>
                                ...
                                <td class='d-none'>Nome do Hotel</td>
                                <td class='d-none'>Hospedagem</td>
                                <td class='d-none'>Hotel</td>
                                <td class='d-none'>Endereço do Hotel</td>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
...
-- JOINS CORRIGIDOS: Primeiro verifica se é coordenador, depois supervisor
LEFT JOIN bdcorp.tbcoord c_diretamente ON sv.matsolicitante = c_diretamente.matricula
LEFT JOIN bdcorp.tbsupervisor s ON sv.matsolicitante = s.matricula
LEFT JOIN bdcorp.tbcoord c_via_supervisor ON s.idtbcoordenador = c_via_supervisor.idtbcoordenador
-- Usa COALESCE para pegar o coordenador (direto ou via supervisor)
LEFT JOIN bdcorp.tbgerente g ON (
    COALESCE(c_diretamente.idtbgerente, c_via_supervisor.idtbgerente) = g.idtbgerente 
    OR sv.matsolicitante = g.matricula
)
LEFT JOIN bdcorp.tbdiretor d ON g.idtbdiretor = d.idtbdiretor
WHERE 1=1";
...
if (!empty($matricula1)) {
    if ($matricula1 == '601002' || $matricula1 == '601004') {
        ...
        // Filtro normal para outras matrículas - ATUALIZADO
...
$sql .= " ORDER BY sv.datasol DESC";
                            ...
                            while ($row = mysqli_fetch_array($resultado, MYSQLI_BOTH)) {
                                ...
                                if ($alimentacao == 0) {
                                    $option_alimentacao = 'Não';
                                    $option_alimentacao_finalizada = 'Não pediu alimentação'; // Definindo a mensagem para ser exibida
                                } elseif ($alimentacao == 1) {
                                    $option_alimentacao = 'Sim, Almoço';
                                } elseif ($alimentacao == 2) {
                                    $option_alimentacao = 'Sim, Almoço e Janta';
                                ...
                                if (!empty($row['idtbmotivo_exclusao'])) {
                                    ...
                                    // Só muda o status se ainda não estiver "Concluido"
                                    if ($status !== 'Concluido') {
                                ...
                                <tr>
                                    ...
                                    <td>$datafim</td>
                                    $botaoValor
                                    <td><a href='uploads/$img_passagem_ida' target='_blank'>$img_passagem_ida</a></td>
                                    <td><a href='uploads/$img_passagem_volta' target='_blank'>$img_passagem_volta</a></td>
                                    <td>$status</td>
                                    <td class='d-none'>$empresa_ida</td>
                                    ...
                                    <td class='d-none'>$empresa_volta</td>
                                    ...
                                    <td class='d-none'>$hotel</td>
                                    <td class='d-none'>$endereco_hotel</td>
                                </tr>";
                                echo "
                    ...
                    <div class='modal-dialog modal-dialog-centered'>
                      <div class='modal-content'>
                        ...
                        <div class='modal-body' style='font-size: 13px;'>
                          ...
                          <p><strong>Valor do Adiantament:</strong> $valor_adiantamento</p>
    ...
    <script type="text/javascript">
        ...
        function baixarExcel() {
            ...
            // Converter a tabela para um objeto SheetJS
            ...
            var mes = String(hoje.getMonth() + 1).padStart(2, '0'); // Mês começa em 0
            ...
            a.href = url;
            a.download = 'HistoricoViagem(' + dataFormatada + ').xlsx'; // Nome do arquivo
            a.click();
