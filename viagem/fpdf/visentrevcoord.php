...
/*$select = $_POST['coordenador'];
$input = $_POST['coordenador1'];
...
if($matfunc==''){
  echo "<script>window.alert('Selecione um funcionário da lista ou insira uma matrícula válida.');
  window.location=\"../gerarrelatorio.php\"</script>";
}*/
...
/*$doismesesatras = date("Y-m-d H:i:s", strtotime($data . "-2 month"));
$doismesesatras = explode(" ", $doismesesatras);
$doismesesatras1 = explode("-", $doismesesatras[0]);
$doismesesatrasf = $doismesesatras1[0]."-".$doismesesatras1[1];

$quatromesesatras = date("Y-m-d H:i:s", strtotime($data . "-4 month"));
$quatromesesatras = explode(" ", $quatromesesatras);
$quatromesesatras1 = explode("-", $quatromesesatras[0]);
$quatromesesatrasf = $quatromesesatras1[0]."-".$quatromesesatras1[1];

$sql = "SELECT * FROM bdcorp.tbpesquisafunc WHERE matriculafunc='$matfunc' AND datahora like '$anomesf%'; ";*/

//$sql = "SELECT * FROM bdcorp.tbpesquisafunc where matriculafunc='$matfunc' AND MONTH(datahora)>=(MONTH('$data')-2) and MONTH(datahora)<=(MONTH('$data')-1);";

$sqla = "SELECT MAX(idtbpesquisafunc) FROM bdcorp.tbpesquisafunc where matriculafunc='$matfunc';";
$resultadoa = mysqli_query($conexao, $sqla) or die(mysqli_error($conexao));
$rowa = mysqli_fetch_array($resultadoa, MYSQLI_BOTH);
  $idtbpesquisafunc = $rowa[0];

$sql = "SELECT * FROM bdcorp.tbpesquisafunc where idtbpesquisafunc='$idtbpesquisafunc';";
...
/*if($linhas<=0){
  echo "<script>window.alert('Não existe entrevista do bimestre anterior com o colaborador especificado.'); window.close();  </script>";
  //echo $anomesf;
