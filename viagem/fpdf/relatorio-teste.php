...
$tpentrevistador = $_POST['tpentrevistador'];
...
$anomes = explode(" ", $hoje);
$anomes1 = explode("-", $anomes[0]);
$anomesf = $anomes1[0]."-".$anomes1[1];

$sql = "SELECT * FROM bdcorp.tbpesquisafunc WHERE matriculafunc='$matfunc' AND datahora like '$anomesf%' AND tipo_entrevistador=$tpentrevistador; ";
...
if($linhas<=0){
  echo "<script>window.alert('Não existe entrevista deste mês com o colaborador especificado.');
  window.location=\"../gerarrelatorio.php\"</script>";
}
...
while($row = mysqli_fetch_array($resultado, MYSQLI_BOTH)){
  ...
  $suportesupervisor = $row['suportesupervisor'];
