                        ...
                        <tbody>
                            ...
                            $sql = "SELECT 
                                        ...
                                        CONCAT(h.nome_hotel,
                                                ...
                                                h.endereco_hotel,
                                                '-',
                                                h.cidade_hotel) AS hotel,
                                        CONCAT(embarque_ida.nome_do_aeroporto,
                                                '-',
                                                embarque_ida.cidade) AS embarque_ida,
                                        CONCAT(desembarque_ida.nome_do_aeroporto,
                                                '-',
                                                desembarque_ida.cidade) AS desembarque_ida,
                                        CONCAT(embarque_volta.nome_do_aeroporto,
                                                '-',
                                                embarque_volta.cidade) AS embarque_volta,
                                        CONCAT(desembarque_volta.nome_do_aeroporto,
                                                '-',
                                                desembarque_volta.cidade) AS desembarque_volta,
                                        sv.matsolicitante,
                                        s.idtbsupervisor,
                                        s.nome AS nome_supervisor,
                                        s.matricula AS matricula_supervisor,
                                        c_diretamente.idtbcoordenador,
                                        c_diretamente.nome AS nome_coordenador,
                                        c_diretamente.matricula AS matricula_coordenador,
                                        g.idtbgerente,
                                    ...
                                    FROM
                                        bdviagem.tbsolviagem sv
                                            LEFT JOIN
                                        bdviagem.tbauxlocalidade f ON sv.idfilial_origem = f.idlocalidade
                                            LEFT JOIN
                                        bdviagem.tbauxlocalidade f_destino ON sv.idfilial_destino = f_destino.idlocalidade
                                            LEFT JOIN
                                        bdviagem.tbmotivo a ON sv.idmotivo = a.idtbmotivo
                                            LEFT JOIN
                                        bdviagem.tbempresas e ON sv.idempresa_ida = e.id_empresa
                                            LEFT JOIN
                                        bdviagem.tbhoteis h ON sv.idhotel = h.idhotel
                                            LEFT JOIN
                                        bdviagem.tblocalviagem AS embarque_ida ON sv.idembarque_ida = embarque_ida.idlocal_viagem
                                            LEFT JOIN
                                        bdviagem.tblocalviagem AS desembarque_ida ON sv.iddesembarque_ida = desembarque_ida.idlocal_viagem
                                            LEFT JOIN
                                        bdviagem.tblocalviagem AS embarque_volta ON sv.idembarque_volta = embarque_volta.idlocal_viagem
                                            LEFT JOIN
                                        bdviagem.tblocalviagem AS desembarque_volta ON sv.iddesembarque_volta = desembarque_volta.idlocal_viagem
                                            LEFT JOIN
                                        bdcorp.tbcoord c_diretamente ON sv.matsolicitante = c_diretamente.matricula
                                            LEFT JOIN
                                        bdcorp.tbsupervisor s ON sv.matsolicitante = s.matricula
                                            LEFT JOIN
                                        bdcorp.tbcoord c_via_supervisor ON s.idtbcoordenador = c_via_supervisor.idtbcoordenador
                                            LEFT JOIN
                                        bdcorp.tbgerente g ON (COALESCE(c_diretamente.idtbgerente,
                                                c_via_supervisor.idtbgerente) = g.idtbgerente
                                            OR sv.matsolicitante = g.matricula)
                                            LEFT JOIN
                                        bdcorp.tbdiretor d ON g.idtbdiretor = d.idtbdiretor
                                    WHERE
                                        1 = 1";

                            if ($_SESSION['perfil'] == 13) {
                            ...
                            if (!empty($dataInicial)) {
                                $sql .= " AND DATE(sv.datasol) >= '$dataInicial'";
                            }

                            if (!empty($dataFinal)) {
                                $sql .= " AND DATE(sv.datasol) <= '$dataFinal'";
                            }
                            ...
                            // Filtro de matrícula (unifica 601002 e 601004, outras mantêm normal)
                            if (!empty($matricula1)) {
                                if ($matricula1 == '601002' || $matricula1 == '601004') {
                                    // Unifica 601002 e 601004
                                    $sql .= " AND ((sv.matsolicitante IN ('601002', '601004') 
                                                OR s.matricula IN ('601002', '601004') 
                                                OR c_diretamente.matricula IN ('601002', '601004') 
                                                OR c_via_supervisor.matricula IN ('601002', '601004') 
                                                OR g.matricula IN ('601002', '601004') 
                                                OR d.matricula IN ('601002', '601004')))";
                                } else {
                                    // Filtro normal para outras matrículas
                                    $sql .= " AND (sv.matsolicitante = '$matricula1' 
                                                OR s.matricula = '$matricula1' 
                                                OR c_diretamente.matricula = '$matricula1' 
                                                OR c_via_supervisor.matricula = '$matricula1' 
                                                OR g.matricula = '$matricula1' 
                            ...
                            while ($row = mysqli_fetch_array($resultado, MYSQLI_BOTH)) {
                                ...
                                if ($alimentacao == 0) {
                                    ...
                                    $option_alimentacao_finalizada = 'Não pediu alimentação';
                                ...
                                if ($adiantamento == 0) {
                                    $adiantamento_option = 'Não';
                                    $option_adiantamento_finalizado = 'Não pediu adiantamento';
                                } elseif ($adiantamento == 1) {
                                    $adiantamento_option = 'Sim';
                                ...
                                $aceite_gegeral = (int) $row['aceite_gegeral'];
                                /* SE FOR 1 É APROVADO PELO GERENTE GERAL, SE FOR 2 NEGADO */
                                if (!empty($row['idtbmotivo_exclusao'])) {
                                    $status = 'Excluído';
                                } else {
                                    if ($row['aceite_do_diretor'] === '0' || $row['aceite_do_diretor'] === 0) {
                                        $status = 'Em Análise';
                                    } else {
                                    ...
                                    // Só muda o status se ainda não estiver "Concluído"
                                    if ($status !== 'Concluído') {
                                ...
                                $imgPassagemIda = !empty($img_passagem_ida) ? "<a href='uploads/$img_passagem_ida' target='_blank'>$img_passagem_ida</a>" : '-';
                                $imgPassagemVolta = !empty($img_passagem_volta) ? "<a href='uploads/$img_passagem_volta' target='_blank'>$img_passagem_volta</a>" : '-';
