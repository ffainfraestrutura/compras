...
$dataInicial = date('Y-m-d', strtotime('-7 days')); // Data inicial é 7 dias antes
$dataFinal = ''; // Data final é o dia de hoje
$statusViagem = '';
$nomeSelecionado = '';
...
LEFT JOIN bdcorp.tbdiretor d ON d.idtbdiretor = g.idtbdiretor
                                ...
                                AND sv.alimentacao != 0
                                AND sv.alimentacao_finalizada IS NULL";
...
$resultado = mysqli_query($conexao, $sql) or die(mysqli_error($conexao));
            ...
            <td><button type='button' class='btn btn-sm btn-primary' data-bs-toggle='modal' data-bs-target='#modalConcluido$id'>Concluido</button></td>
        </tr>";
                  ...
                  <div class='modal fade' id='modalConcluido$id' tabindex='-1' aria-labelledby='exampleModalLabel' aria-hidden='true' enctype='multipart/form-data'>
                     <div class='modal-dialog modal-dialog-centered'>
                        <div class='modal-content'>
                          <div class='modal-header'>
                          <h5 class='modal-title' id='modalDetalhesLabel$id'>Alimentação</h5>
                          <button type='button' class='btn-sm btn-close' data-bs-dismiss='modal' aria-label='Fechar'></button>
                          </div>
                          <div class='modal-body' style='font-size: 13px;'>
                          <h6>Você já concluiu o Email referente a alimentação desse colaborador?</h6>
                          </div>
                        <div class='modal-footer'>
                       <form method='post' action='./control/alimentacao_finalizada.php'>
                            <input type='hidden' name='idtbsolviagem' value='$id'>
                            <button type='submit' class='btn btn-success' name='alimentacao_finalizada' value='1'>Sim</button>
                        </form>
                            <button type='button' class='btn btn-secondary' data-bs-dismiss='modal'>Fechar</button>
                        </div>
                      </div>
