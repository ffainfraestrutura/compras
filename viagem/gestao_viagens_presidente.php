...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao'], $_POST['idtbsolviagem'])) {
    ...
    if ($id > 0 && ($acao === 'aprovar' || $acao === 'negar')) {
        if ($acao === 'aprovar') {
            ...
            $aceiteCoo = 1;
            $aceiteGeral = 3;
            $obsGerente = 'Aprovado automaticamente via tela Gestão de viagens. (' . $matricula1 . ')';
            $obsDiretor = 'Aprovado automaticamente via tela Gestão de viagens. (' . $matricula1 . ')';
        } else {
            $aceiteDiretor = 2;
            $aceiteCoo = 0;
            $aceiteGeral = 2;
            $obsGerente = 'Negado automaticamente via tela Gestão de viagens. (' . $matricula1 . ')';
            $obsDiretor = 'Negado automaticamente via tela Gestão de viagens. (' . $matricula1 . ')';
        }
...
$filtroEtapa = trim($_GET['etapa'] ?? '');
$filtroAplicado = false;

// Verificar se foi enviado filtro customizado
if (isset($_GET['data_inicio']) || isset($_GET['data_fim']) || isset($_GET['etapa'])) {
    $filtroAplicado = true;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Solicitações de Viagem - Gestão Presidente</title>
...
<div id="layoutSidenav_content">
    <main style="width: 100%;" class="mb-2">
        <div class="container-fluid px-4 col-sm-12">
            <h1 class="h1 pt-3 pb-2 col-12 text-center">Solicitações de Viagem - Gestão Presidente</h1>
            ...
            <form method="get" class="row g-3 align-items-end mt-3">
                ...
                <div class="col-md-2">
                    ...
                    <select class="form-select" id="etapa" name="etapa">
                        ...
                        <option value="gerente" <?php echo ($filtroEtapa === 'gerente') ? 'selected' : ''; ?>>Gerente</option>
                        <option value="diretor" <?php echo ($filtroEtapa === 'diretor') ? 'selected' : ''; ?>>Diretor</option>
                        <option value="coo_cfo" <?php echo ($filtroEtapa === 'coo_cfo') ? 'selected' : ''; ?>>COO/CFO</option>
                    </select>
                </div>
                <div class="col-md-3 d-flex gap-2">
                    <button type="submit" class="btn btn-primary w-100">Filtrar</button>
                    <a href="gestao_viagens_presidente.php" class="btn btn-outline-secondary w-100">Limpar</a>
                </div>
            </form>
                ...
                <table id='tabela1' class='table' style="font-size: 13px;">
                    ...
                    <tbody>
                        ...
                        while ($row = mysqli_fetch_array($resultado, MYSQLI_BOTH)) {
                            ...
                            // Calcular status com sincronização ao histórico
                            if (!empty($row['idtbmotivo_exclusao'])) {
                                $status = 'Excluído';
                            } else {
                                if ($row['aceite_do_diretor'] === 0 || $row['aceite_do_diretor'] === NULL) {
                                    $status = 'Em Análise';
                                } else {
                                ...
                                // Verifica aprovações do COO/CFO se não for Concluído
                            ...
                            // Verificar se é pendente (falta aprovação em qualquer etapa)
                            $isPendente = false;
                            $etapasAbertas = []; // Armazena todas as etapas pendentes
                            
                            // Verifica pendência em DIRETOR
                            if ($row['aceite_do_diretor'] === NULL || $row['aceite_do_diretor'] === '0') {
                                $isPendente = true;
                                $etapasAbertas[] = 'diretor';
                            }
                            
                            // Verifica pendência em COO/CFO
                            if ($row['aprovado_coo'] === NULL || $row['aprovado_coo'] === '') {
                                $isPendente = true;
                                $etapasAbertas[] = 'coo_cfo';
                            }
                            
                            // Exibir apenas solicitações pendentes (falta em qualquer etapa)
                            if (!$isPendente) {
                                continue;
                            }
                            
                            // Construir status descrevendo todas as etapas pendentes
                            $statusPorEtapa = 'Pendente: ' . implode(', ', array_map(function($etapa) {
                                return $etapa === 'diretor' ? 'Diretor' : 'COO/CFO';
                            }, $etapasAbertas));
                            
                            // Aplicar filtro por etapa se especificado
                            if ($filtroEtapa !== '') {
                                if (!in_array($filtroEtapa, $etapasAbertas)) {
                                    continue; // Pula se a etapa filtrada não está pendente
                                }
                            }

                            // Atualizar o status a ser exibido
                            $status = $statusPorEtapa;
