...
<body class="sb-nav-fixed">
    ...
    CONCAT(sv.idempresa_volta, ' - ', e2.nome_empresa) AS empresa_volta,
...
FROM
    bdviagem.tbsolviagem sv
        LEFT JOIN bdviagem.tbauxlocalidade f 
            ON sv.idfilial_origem = f.idlocalidade
        LEFT JOIN bdviagem.tbauxlocalidade f_destino 
            ON sv.idfilial_destino = f_destino.idlocalidade
        LEFT JOIN bdviagem.tbmotivo a 
            ON sv.idmotivo = a.idtbmotivo
        LEFT JOIN bdviagem.tbempresas e 
            ON sv.idempresa_ida = e.id_empresa
        LEFT JOIN bdviagem.tbempresas e2 
            ON sv.idempresa_volta = e2.id_empresa
        LEFT JOIN bdviagem.tbhoteis h 
            ON sv.idhotel = h.idhotel
        LEFT JOIN bdviagem.tblocalviagem embarque_ida 
            ON sv.idembarque_ida = embarque_ida.idlocal_viagem
        LEFT JOIN bdviagem.tblocalviagem desembarque_ida 
            ON sv.iddesembarque_ida = desembarque_ida.idlocal_viagem
        LEFT JOIN bdviagem.tblocalviagem embarque_volta 
            ON sv.idembarque_volta = embarque_volta.idlocal_viagem
        LEFT JOIN bdviagem.tblocalviagem desembarque_volta 
            ON sv.iddesembarque_volta = desembarque_volta.idlocal_viagem
        LEFT JOIN bdcorp.tbsupervisor s 
            ON sv.matsolicitante = s.matricula
        LEFT JOIN bdcorp.tbcoord c 
            ON s.idtbcoordenador = c.idtbcoordenador
        LEFT JOIN bdcorp.tbgerente g 
            ON c.idtbgerente = g.idtbgerente
        LEFT JOIN bdcorp.tbdiretor d 
            ON g.idtbdiretor = d.idtbdiretor
        LEFT JOIN bdcorp.tbfuncionario f_gerente 
            ON sv.matricula_aceite_gerente = f_gerente.matricula
WHERE
    (EXISTS( SELECT 
            1
        FROM
...
";
                            ...
                            while ($row = mysqli_fetch_array($resultado, MYSQLI_BOTH)) {
                                ...
                                $aceiteDiretor = "<td>
                                                    <form method='post' action='./control/atualizar_aceitegegeral.php'>
